...
export class TaskService {
    ...
    async updateTask(id, workspaceId, data, userId) {
        return runInTransaction(async (tx) => {
    ...
    async deleteTask(id, workspaceId, userId) {
        return runInTransaction(async (tx) => {
            const existing = await taskRepository.findById(id, tx);
            if (!existing || existing.deletedAt || existing.workspaceId !== workspaceId) {
                throw new Error('Task not found');
    ...
    async reorderTask(id, workspaceId, data, userId) {
        return runInTransaction(async (tx) => {
            const existing = await taskRepository.findById(id, tx);
            if (!existing || existing.deletedAt || existing.workspaceId !== workspaceId) {
                throw new Error('Task not found');
    ...
    async completeTask(id, workspaceId, userId) {
        return runInTransaction(async (tx) => {
            const existing = await taskRepository.findById(id, tx);
            if (!existing || existing.deletedAt || existing.workspaceId !== workspaceId) {
                throw new Error('Task not found');
