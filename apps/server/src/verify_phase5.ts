...
function computeGoalPace(goal: any) {
  ...
  if (goal.snapshots && goal.snapshots.length >= 2) {
    const sorted = [...goal.snapshots].sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  let status = 'on_track';
  if (daysRemaining === 0 && goal.progress < 100) status = 'past_due';
  else if (actualPace === 0 && goal.progress < 100) status = 'stalled';
  else if (actualPace < requiredPace) status = 'behind';
  else if (actualPace > requiredPace * 1.2) status = 'ahead';
