...
async function computeAndVerifyStreak(habitId) {
    ...
    const completedDates = new Set(habit.completions.filter(c => c.completed).map(c => toDateStr(new Date(c.date))));
    let currentStreak = 0;
...
function generate30DayHeatmap(habit) {
    const days = [];
    ...
    for (let i = 29; i >= 0; i--) {
        ...
        const completed = habit.completions?.some((c) => toDateStr(new Date(c.date)) === dStr && c.completed);
...
async function verifyPhase6() {
    ...
    if (!hydratedHabit)
        throw new Error('Hydrated habit missing');
...
//# sourceMappingURL=verify_phase6.js.map