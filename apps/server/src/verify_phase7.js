...
async function step1_PageFullTextSearch() {
    ...
    const pageResults = searchResult.results.filter((r) => r.type === 'page');
...
async function step3_DecisionLogCRUD() {
    ...
    const found = afterDelete.find((d) => d.id === decision.id);
    if (!found) {
...
async function step4_CascadeDeleteRestore() {
    ...
    const orphanedIssue1 = allIssues.find((i) => i.id === issue1.id);
    const orphanedIssue2 = allIssues.find((i) => i.id === issue2.id);
    ...
    const orphanedSprint = allSprints.find((s) => s.id === sprint.id);
    if (!orphanedSprint) {
    ...
    const backIssue1 = restoredIssues.find((i) => i.id === issue1.id);
    const backIssue2 = restoredIssues.find((i) => i.id === issue2.id);
    ...
    const backSprint = restoredSprints.find((s) => s.id === sprint.id);
    if (backSprint) {
...
export {};
//# sourceMappingURL=verify_phase7.js.map