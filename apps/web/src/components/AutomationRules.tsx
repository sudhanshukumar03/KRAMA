...
export function AutomationRules() {
 ...
 <Zap className="w-6 h-6 text-warning fill-amber-500/20" /> Automations
 </h1>
 ...
 <div className="w-16 h-16 rounded-2xl bg-warning-tint text-warning flex items-center justify-center mx-auto mb-4">
...
function RuleCard({ rule, onToggle, onDelete }: { rule: any; onToggle: (id: string, isActive: boolean) => void; onDelete: (id: string) => void }) {
 return (
 <div className="v4-card border border-border bg-surface-hover/30 p-5 rounded-2xl flex items-center justify-between group">
 <div className="flex items-start gap-4">
 <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${rule.isActive ? 'bg-warning-tint text-warning' : 'bg-surface border border-border text-muted'}`}>
 ...
 <span className="text-success font-bold">{rule.actionType}</span>
 </div>
 </div>
 </div>
 ...
 <input type="checkbox" checked={rule.isActive} onChange={(e) => onToggle(rule.id, e.target.checked)} className="sr-only peer" />
 <div className="w-9 h-5 bg-surface border border-border peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-background after:border-border after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-warning-tint peer-checked:border-warning-tint"></div>
 </label>
 <button onClick={() => onDelete(rule.id)} className="w-8 h-8 rounded-lg flex items-center justify-center text-secondary hover:text-[#DC2626] hover:bg-error-tint transition-colors opacity-0 group-hover:opacity-100">
...
function RuleCreateModal({ onClose }: { onClose: () => void }) {
 ...
 <h3 className="font-semibold text-primary flex items-center gap-2"><Zap className="w-4 h-4 text-warning" /> New Automation</h3>
 </div>
 ...
 <h4 className="text-sm font-semibold text-primary flex items-center gap-2"><Play className="w-4 h-4 text-success" /> WHEN</h4>
 ...
 <div className="p-4 border border-border rounded-xl bg-warning-tint space-y-4">
 <h4 className="text-sm font-semibold text-primary flex items-center gap-2"><Settings2 className="w-4 h-4 text-warning" /> THEN</h4>
