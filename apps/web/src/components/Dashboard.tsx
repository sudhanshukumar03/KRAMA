...
export function Dashboard() {
 ...
 {/* AI Insight */}
 <div className="bg-gradient-to-r from-blue-600/10 to-blue-400/10 border border-accent/20 rounded-2xl p-6 relative overflow-hidden group shadow-sm hover:shadow-md transition-shadow flex flex-col">
 ...
 <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center shrink-0 shadow-inner">
 ...
 Daily Insight
 {(insightLoading || regeneratingInsight) && <span className="inline-block w-4 h-4 rounded-full border-2 border-accent/20 border-t-blue-600 animate-spin" />}
 </h3>
 <button 
 onClick={handleRegenerateInsight}
 disabled={insightLoading || regeneratingInsight}
 className="text-xs font-medium text-accent bg-accent/10 hover:bg-accent-hover/20 px-2 py-1 rounded-md flex items-center gap-1.5 transition-colors disabled:opacity-50 shrink-0 ml-2"
 >
 ...
 <button onClick={() => window.location.hash = '#/planner'} className="text-sm font-medium text-accent hover:text-accent px-3 py-1.5 rounded-lg bg-accent/10 hover:bg-accent/20 transition-colors">
 Open Planner
 </button>
 </div>
 ...
 <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-sm ${topPriorityTask.priority === 'URGENT' ? 'bg-error-tint text-error' : 'bg-accent/10 text-accent'}`}>
 ...
 <span className="text-sm font-medium text-primary truncate max-w-[180px] group-hover:text-accent transition-colors">{p.name}</span>
 ...
 <div className="h-full bg-accent transition-all duration-500" style={{ width: `${p.progress || 0}%` }} />
 </div>
 </div>
 </div>
 ))}
 ...
 {[
 { id: 'note', icon: FileText, label: 'Note', color: 'bg-accent/10 text-accent' },
 { id: 'task', icon: CheckSquare, label: 'Task', color: 'bg-accent/10 text-accent' },
 { id: 'idea', icon: Lightbulb, label: 'Idea', color: 'bg-warning-tint text-warning' },
 { id: 'link', icon: LinkIcon, label: 'Link', color: 'bg-accent/10 text-blue-800' }
 ].map(item => (
 <button
 key={item.id}
 ...
 <div key={task.id} className={`p-3 border rounded-xl flex items-start gap-3 transition-colors ${task.status === 'DONE' ? 'bg-surface-hover border-transparent' : 'bg-surface border-border hover:border-accent/20'}`}>
 <button 
 onClick={() => toggleTaskMutation.mutate(task)}
 disabled={toggleTaskMutation.isPending}
 className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center shrink-0 transition-colors ${task.status === 'DONE' ? 'bg-accent border-accent/20 text-white' : 'border-secondary hover:border-accent/20'}`}
 >
 ...
 <span className={`inline-block mt-1 text-[9px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded-sm ${task.priority === 'URGENT' ? 'bg-error-tint text-error' : 'bg-accent/10 text-accent'}`}>
 {task.priority}
 </span>
 )}
 </div>
