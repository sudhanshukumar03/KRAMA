...
export function LocationSettingsModal({ open, onClose, currentCountry, currentRegion }: Props) {
 ...
 <div className="bg-surface border border-border rounded-2xl w-full max-w-sm shadow-2xl animate-in slide-in-from-bottom-4 duration-200" onClick={e => e.stopPropagation()}>
 <div className="flex items-center justify-between px-5 py-4 border-b border-border">
 <h3 className="font-semibold text-primary">Calendar Location</h3>
 <button onClick={onClose} className="text-muted hover:text-primary bg-surface-hover hover:bg-surface-hover rounded-lg p-1.5">
 <X size={16} />
 </button>
 </div>
 ...
 className="w-full px-3 py-2 rounded-xl border border-border focus:outline-none focus:border-accent/20 text-sm bg-surface"
 >
 ...
 <div className="bg-slate-50 rounded-xl p-3 text-xs text-muted mt-2 border border-border">
 <div className="font-semibold mb-1 text-primary">Timezone</div>
 Intelligently derived as <span className="font-mono text-accent bg-blue-50 px-1 py-0.5 rounded">Asia/Kolkata</span>
 </div>
 ...
 className="w-full bg-accent text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-accent-hover transition-colors"
 >
 Save Changes
 </button>
 </div>
