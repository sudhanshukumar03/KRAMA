...
const FOCUS_THEMES = [
  { id: 'sprint', label: '25m Focus Sprint', mins: 25, icon: Rocket, color: 'border-indigo-500/50 hover:border-indigo-500 bg-indigo-500/10 text-indigo-300', activeColor: 'bg-indigo-600 text-white border-indigo-500 ring-2 ring-indigo-500/50', badge: 'POMODORO SPRINT', desc: 'High-intensity Pomodoro sprint for rapid code execution' },
  { id: 'deep', label: '45m Deep Work', mins: 45, icon: Brain, color: 'border-amber-500/50 hover:border-amber-500 bg-amber-500/10 text-amber-300', activeColor: 'bg-amber-600 text-white border-amber-500 ring-2 ring-amber-500/50', badge: 'COGNITIVE IMMERSION', desc: 'Sustained focus for complex architecture & refactoring' },
  { id: 'quick', label: '15m Quick Pulse', mins: 15, icon: Zap, color: 'border-emerald-500/50 hover:border-emerald-500 bg-emerald-500/10 text-emerald-300', activeColor: 'bg-emerald-600 text-white border-emerald-500 ring-2 ring-emerald-500/50', badge: 'RAPID REFACTOR', desc: 'Short maintenance burst, PR reviews, and bug squashing' },
  { id: 'stopwatch', label: 'Open Stopwatch', mins: 0, icon: Clock, color: 'border-sky-500/50 hover:border-sky-500 bg-sky-500/10 text-sky-300', activeColor: 'bg-sky-600 text-white border-sky-500 ring-2 ring-sky-500/50', badge: 'UNBOUNDED FLOW', desc: 'Count up indefinitely without time constraints' },
  { id: 'custom', label: 'Custom Timer', mins: -1, icon: Settings, color: 'border-purple-500/50 hover:border-purple-500 bg-purple-500/10 text-purple-300', activeColor: 'bg-purple-600 text-white border-purple-500 ring-2 ring-purple-500/50', badge: 'CUSTOM TARGET', desc: 'Set your own custom minutes target for this session' }
];
...
export function DailyReview() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full bg-canvas min-h-full animate-in fade-in duration-150 pb-24">
        ...
        <div className="fixed inset-0 z-50 bg-[#0B0F19] flex flex-col items-center justify-center text-white animate-in fade-in zoom-in-95 duration-200 p-6 select-none">
          ...
          <div className="flex flex-col items-center max-w-2xl w-full text-center">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs font-mono uppercase tracking-[0.2em] text-[#9CA3AF] flex items-center gap-2">
            ...
            <p className="text-xs sm:text-sm text-[#9CA3AF] mb-12 max-w-md font-mono">
      ...
      <div className="mb-8 bg-[#111827] rounded-2xl p-6 md:p-7 text-white shadow-xl border border-white/10 space-y-6">
        {/* Header Strip */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-white/10">
          <div className="flex items-center gap-3.5">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 flex items-center justify-center shrink-0 border border-indigo-500/30 shadow-inner">
              <Brain className="w-5 h-5 text-indigo-400 stroke-[1.75]" />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <h2 className="text-lg font-bold text-white tracking-tight">Theme-Based Focus Timer</h2>
              ...
              <p className="text-xs text-[#9CA3AF]">Set timed sprints or open stopwatches to execute your tasks with precision.</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-start sm:self-auto">
            <button 
              ...
              className="text-amber-400 hover:text-amber-300 font-mono text-xs flex items-center gap-1.5 bg-white/10 hover:bg-white/15 px-3 py-1.5 rounded-lg transition-colors cursor-pointer border border-white/10"
              title="Enter Immersive Full-Screen Mode"
            >
              <Maximize2 className="w-3.5 h-3.5" /> Full Screen
            </button>
            <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 px-3 py-1.5 rounded-lg flex items-center gap-2 text-amber-300 text-xs font-mono font-bold">
              <Trophy className="w-3.5 h-3.5 text-amber-400" /> Today's Total: {Math.floor(secondsElapsed / 3600)}h {Math.floor((secondsElapsed % 3600) / 60)}m
            </div>
          </div>
        </div>

        {/* Target Task Selection Bar ("Set time to do this thing") */}
        <div className="bg-white/5 border border-white/10 rounded-xl p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative">
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            <Target className="w-4 h-4 text-indigo-400 shrink-0 animate-pulse" />
            <span className="text-xs font-mono font-bold text-[#9CA3AF] uppercase tracking-wider shrink-0">Focusing On:</span>
            <input 
              type="text"
              value={focusTask}
              onChange={(e) => setFocusTask(e.target.value)}
              placeholder="What are you executing right now? e.g. System Refactor, Unit Tests..."
              className="flex-1 bg-transparent border-none text-sm text-white font-medium focus:outline-none placeholder:text-gray-500 min-w-0 font-sans"
            />
          </div>
          ...
          <div className="relative shrink-0">
            <button
              ...
              className="px-3 py-1.5 bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-200 border border-indigo-500/40 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5 cursor-pointer"
            >
              <ListTodo className="w-3.5 h-3.5" /> Pick Workspace Task <ChevronDown className="w-3 h-3 ml-0.5" />
            </button>

            {showTaskDropdown && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-[#1E293B] border border-white/20 rounded-xl shadow-2xl p-2 z-30 max-h-64 overflow-y-auto space-y-1 text-left animate-in fade-in zoom-in-95 duration-150">
                <div className="text-[10px] font-mono uppercase font-bold text-gray-400 px-2 py-1 border-b border-white/10">Active Issues & Habits</div>
                {issues.map(issue => (
                  <button
                    key={issue.id}
                    onClick={() => {
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs text-gray-200 hover:bg-indigo-600/30 hover:text-white truncate transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" /> {issue.title}
                  </button>
                ))}
                {habits.map(habit => (
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs text-gray-200 hover:bg-emerald-600/30 hover:text-white truncate transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" /> {habit.name}
                  </button>
                ))}
                {projects.map(proj => (
                  <button
                    ...
                    className="w-full text-left px-2.5 py-1.5 rounded-lg text-xs text-gray-200 hover:bg-purple-600/30 hover:text-white truncate transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    <FolderKanban className="w-3 h-3 text-purple-400 shrink-0" /> {proj.name}
                  </button>
                ))}
                {issues.length === 0 && habits.length === 0 && projects.length === 0 && (
                  <div className="text-xs text-gray-400 px-2 py-2 italic text-center">No active tasks found. Type above!</div>
                )}
              </div>
            )}
          </div>
        ...
        <div className="space-y-2">
          <div className="text-xs font-mono font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-amber-400" /> Select Timer Theme
          </div>
            ...
            {FOCUS_THEMES.map(t => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "p-3 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between h-full relative overflow-hidden group",
                    isSelected ? t.activeColor : `${t.color} hover:bg-white/10`
                  )}
                >
                  <div className="flex items-center justify-between mb-2">
                    <Icon className={cn("w-4 h-4", isSelected ? "text-white" : "text-gray-300 group-hover:scale-110 transition-transform")} />
                  ...
                  <div>
                    ...
                    <div className={cn("text-[10px] line-clamp-1 leading-snug", isSelected ? "text-white/80" : "text-gray-400")}>
                      {t.desc}
                    </div>
                  </div>
                </button>
          ...
          {activeThemeId === 'custom' && (
            <div className="flex items-center gap-3 pt-1 animate-in fade-in duration-150">
              <span className="text-xs font-mono text-gray-300">Custom Target Minutes:</span>
              <input
                type="number"
                min={1}
                max={480}
                ...
                className="w-20 px-3 py-1 bg-white/10 border border-white/20 rounded-lg text-sm text-white font-mono font-bold text-center focus:outline-none focus:border-purple-500"
              />
              <span className="text-xs text-gray-400 font-sans">minutes (${customMins * 60} seconds target)</span>
            </div>
          )}
        </div>

        {/* Active Display & Controls Strip */}
        <div className="bg-gradient-to-r from-white/5 via-white/[0.07] to-white/5 border border-white/15 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-inner">
          <div className="text-center sm:text-left z-10">
            <div className="text-[11px] font-mono uppercase tracking-widest text-indigo-300 mb-1 flex items-center justify-center sm:justify-start gap-2 font-bold">
              <Clock className="w-3.5 h-3.5 text-indigo-400 animate-pulse" />
              {activeThemeId === 'stopwatch' ? 'Open Elapsed Stopwatch' : `Remaining — ${activeTheme.label}`}
            </div>
            <div className="text-5xl sm:text-6xl font-mono font-black tracking-wider text-white drop-shadow-md my-1">
              {displayTime}
            </div>
            <div className="text-xs font-mono text-gray-400 flex items-center justify-center sm:justify-start gap-2 mt-1">
              <span>Session Logged: <strong className="text-white">{Math.floor(sessionSeconds / 60)}m {sessionSeconds % 60}s</strong></span>
              <span>•</span>
              <span className="truncate max-w-[200px] text-gray-300">{focusTask}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 z-10 w-full sm:w-auto justify-center">
            <button
              ...
              className={cn(
                "flex-1 sm:flex-none px-6 py-3.5 rounded-xl font-mono text-sm font-bold flex items-center justify-center gap-2.5 transition-all cursor-pointer shadow-lg hover:scale-105 active:scale-95",
            ...
            <button
              ...
              className="w-12 h-12 rounded-xl bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-all cursor-pointer shrink-0 hover:scale-105"
              title="Reset Timer"
            >
