...
content = """import { format, parseISO, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, isToday } from "date-fns";
  ...
  return (
    <div className="flex-1 flex gap-8 min-h-0 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-0 px-6 pb-6">
        <div className="grid grid-cols-7 gap-0 text-center text-[11px] mb-2 flex-shrink-0">
          {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d, i) => (
            <div key={i} className="font-bold text-slate-400 uppercase tracking-wider py-2 border-b border-slate-100">{d}</div>
          ))}
        </div>

        <div className="grid grid-cols-7 auto-rows-fr gap-0 flex-1 overflow-hidden border-l border-t border-slate-100">
          {monthDays.map((day, idx) => {
            ...
            return (
              <div 
                key={day.toISOString()} 
                className={`flex flex-col min-h-0 p-1.5 border-r border-b border-slate-100 transition-colors overflow-hidden ${
      ...
      <div className="w-72 flex-shrink-0 flex flex-col gap-4 overflow-y-auto hide-scrollbar pr-6 pb-6">
        ...
        {/* UPCOMING HOLIDAYS */}
        <div className="flex flex-col bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            <CalendarIcon size={16} className="text-blue-500" />
            <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">Upcoming Holidays</h3>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {isLoading ? (
              ...
              <div className="text-sm text-slate-400 py-4 text-center italic border border-dashed border-slate-200 rounded-xl">No upcoming holidays.</div>
            ) : (
              upcomingHolidays.slice(0, 4).map((h: any) => (
                <div key={h.id} className="flex gap-3 items-start p-2 rounded-xl border border-transparent hover:bg-slate-50 hover:border-slate-100 transition-colors cursor-default">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-700 leading-tight">{h.name}</div>
                    <div className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider mt-0.5">{h.type.replace(/_/g, " ")}</div>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="flex flex-col bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">Categories</h3>
          </div>
          ...
          <div className="flex flex-col gap-1">
            {[
              ...
              { label: "Optional Holidays", color: "text-slate-500", bg: "bg-slate-50", checked: false }
            ].map((cat) => (
              <label key={cat.label} className="flex items-center justify-between cursor-pointer p-1.5 hover:bg-slate-50 rounded-lg transition-colors group">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-[12px] font-semibold text-slate-700 group-hover:text-slate-900">{cat.label}</span>
                </div>
                <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-colors ${cat.checked ? "bg-blue-600 border-blue-600" : "bg-white border-slate-300"}`}>
                  {cat.checked ? (
        ...
        <div className="flex flex-col bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[11px] font-bold text-slate-800 uppercase tracking-wider">Sources</h3>
            </div>
