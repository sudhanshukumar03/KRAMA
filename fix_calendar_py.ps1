        ...
        <div className="grid grid-cols-7 auto-rows-fr gap-0 flex-1 overflow-hidden border-l border-t border-slate-100">
          {monthDays.map((day, idx) => {
            ...
            return (
              <div 
                key={day.toISOString()} 
                className={lex flex-col min-h-0 p-1.5 border-r border-b border-slate-100 transition-colors overflow-hidden  }
              >
                <div className="flex justify-between items-start mb-0.5">
                  <span className={	ext-[12px] font-bold w-6 h-6 flex items-center justify-center rounded-full }>
                    {format(day, "d")}
                  </span>
                </div>
                ...
                <div className="flex-1 flex flex-col gap-0.5 overflow-y-auto hide-scrollbar mt-0.5">
                  {dayHolidays.map((h: any) => (
                    <div 
                      key={h.id} 
                      className={	ext-[9px] font-bold px-1.5 py-0.5 rounded leading-tight border }
                    >
                      {h.name}
                    </div>
                  ))}
