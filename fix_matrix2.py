...
const BLOCK_COLORS: Record<string, string> = {
  MEETING:  "bg-purple-50 border-purple-400 text-purple-800",
  PERSONAL: "bg-orange-50 border-orange-400 text-orange-800",
  STUDY:    "bg-emerald-50 border-emerald-400 text-emerald-800",
  WORK:     "bg-blue-50 border-blue-400 text-blue-800",
  HEALTH:   "bg-rose-50 border-rose-400 text-rose-800",
  ADMIN:    "bg-gray-50 border-gray-400 text-gray-800",
...
const MATRIX_COLLAPSE_STORAGE_KEY = "krama.planner.matrix.expanded.v2";
...
export function PlannerMatrix({
  ...
  return (
    <>
      <section className="overflow-hidden bg-white shadow-sm flex flex-col w-full h-full relative">
        <div className="w-full flex flex-col flex-1 min-h-0">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden flex flex-col hide-scrollbar">
            ...
            >
              <div className="grid grid-cols-[160px_repeat(7,minmax(0,1fr))] w-full">
                <div className="border-r border-border" />
                {days.map((day) => {
                  const dayHols = holidayBlocks.filter(b => isSameDay(parseISO(b.date), day));
                  return (
                    <div key={dateKey(day)} className="border-r border-border last:border-r-0 p-1.5 flex flex-col gap-1 items-center justify-center min-h-[40px]">
                      {dayHols.map(h => (
                      ...
                      {dayHols.length === 0 && <span className="text-slate-300">-</span>}
                    </div>
                  );
                })}
              </div>
            ...
            <MatrixRow 
              ...
              flexClass="flex-1 min-h-0"
            >
              <div className="grid grid-cols-[160px_repeat(7,minmax(0,1fr))] w-full h-full">
                <div className="border-r border-border flex flex-col gap-1 p-2">
                   <div className="text-[10px] text-muted-foreground font-medium">{data.tasks.length} tasks</div>
                </div>
                {days.map((day) => {
                  ...
                  return (
                    <div key={dateKey(day)} className="border-r border-border last:border-r-0 p-2 flex flex-col gap-1.5 overflow-y-auto hide-scrollbar">
                      ...
                      {dayTasks.length === 0 && <div className="text-center text-slate-300 mt-2">-</div>}
                    </div>
                  );
                })}
              </div>
                    ...
                    <div key={dateKey(day)} className="border-r border-border last:border-r-0 p-1.5 flex flex-col gap-1.5 overflow-y-auto hide-scrollbar">
                      {dayBlocks.map((block) => (
                        <button
                          ...
                          className={"w-full text-left rounded-lg p-2 border-l-4 border-t border-r border-b shadow-sm transition-all hover:shadow-md cursor-pointer flex flex-col gap-1 " + (BLOCK_COLORS[block.type] || BLOCK_COLORS.OTHER)}
                        >
                      ...
                      {dayBlocks.length === 0 && <div className="text-center text-slate-300 mt-2">-</div>}
                    </div>
                  );
                })}
              </div>
            ...
            >
              <div className="grid grid-cols-[160px_repeat(7,minmax(0,1fr))] w-full h-full">
                <div className="border-r border-border flex flex-col p-2 text-[10px] text-muted-foreground font-medium">
                ...
                {days.map((day) => {
                  ...
                  return (
                    <div key={dateKey(day)} className="border-r border-border last:border-r-0 p-1.5 flex flex-col gap-1.5">
                      ...
                      {dayMilestones.length === 0 && <div className="text-center text-slate-300">-</div>}
                    </div>
                  );
                })}
              </div>
          ...
          {/* LEGEND */}
          <div className="flex-shrink-0 border-t border-border bg-slate-50/50 p-2 flex items-center justify-center gap-4 flex-wrap text-[9px] font-bold uppercase tracking-wider text-slate-500">
...
function MatrixRow({
  ...
  return (
    <div className={\lex flex-col border-b border-border w-full \\}>
      {/* If expanded, we merge the title button with the grid to act as a proper row */}
      {!isExpanded ? (
        <button
          type="button"
          onClick={onToggle}
          className="p-3 flex items-center text-left hover:bg-muted/10 transition-colors w-full"
        >
        ...
        <div className="flex flex-1 min-h-0 w-full relative">
          <button
            type="button"
            onClick={onToggle}
            className="absolute top-0 left-0 w-[160px] p-3 flex flex-col text-left hover:bg-muted/10 transition-colors z-10"
          >
          ...
          <div className="flex-1 min-w-0 w-full flex flex-col pt-12">
            {children}
          </div>
        </div>
      )}
...
with open('apps/web/src/components/planner/PlannerMatrix.tsx', 'w') as f:
    f.write(code)
